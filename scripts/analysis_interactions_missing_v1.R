...
# Make NA's ------------------------------
X_NA <- matrix(data = rbinom(n = nrow(X) * ncol(X), size = 1, prob = 0.2), nrow = nrow(X), ncol = ncol(X))
Y_NA <- matrix(data = rbinom(n = nrow(Y) * ncol(Y), size = 1, prob = 0.2), nrow = nrow(Y), ncol = ncol(Y))

# Make LOD's --------------------------------
LOD <-  mean(apply(abs(X), 1, min))
