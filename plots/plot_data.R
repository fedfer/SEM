...
ind <- Cor_plot[,3] != 1
Cor_plot <- Cor_plot[ind,] 
Cor_plot[,3] %>% mean(. , na.rm = T)
...
# COR Y
C_out <- cor(df_out_analysis[,-1], use = "pairwise.complete")
C_out_eig <- C_out %>% eigen()
cumsum(C_out_eig$values)/sum(C_out_eig$values)
